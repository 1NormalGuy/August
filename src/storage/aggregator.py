...
# 数据源配置
SOURCES_CONFIG = {
    "cailian": {"name": "财联社", "order": 1},
    "wallstreetcn": {"name": "华尔街见闻", "order": 2},
    "baidu": {"name": "百度热搜", "order": 3},
    "jin10": {"name": "金十数据", "order": 4},
    "ifeng": {"name": "凤凰网", "order": 5},
    "toutiao": {"name": "今日头条", "order": 6},
    "bloomberg": {"name": "彭博社", "order": 7},
    "jiqizhixin": {"name": "机器之心", "order": 8},
}
...
class DailyAggregator:
    ...
    def __init__(self, temp_path: Optional[Path] = None, output_path: Optional[Path] = None):
        from src.core import cfg
        self.temp_path = temp_path or cfg.temp_dir
        self.output_path = output_path or cfg.data_dir

    def generate(self, date: str) -> Optional[Path]:
        """
        生成指定日期的汇总文件
